...
// }
//SJM
// bool JsSelectionUtils::Equals(napi_env env, napi_value value, napi_ref copy, std::thread::id threadId)
// {
//     if (copy == nullptr) {
...
// void *JsSelectionUtils::GetNativeSelf(napi_env env, napi_callback_info info)
// {
...
// napi_status JsSelectionUtils::GetValue(napi_env env, napi_value in, int32_t &out)
// {
...
// /* napi_value <-> uint32_t */
// napi_status JsSelectionUtils::GetValue(napi_env env, napi_value in, uint32_t &out)
// {
//     napi_valuetype type = napi_undefined;
//     napi_status status = napi_typeof(env, in, &type);
//     CHECK_RETURN((status == napi_ok) && (type == napi_number), "invalid type", napi_generic_failure);
...
napi_status JsSelectionUtils::GetValue(napi_env env, napi_value in, PanelInfo &out)
{
    ...
    if (status == napi_ok) {
        JsUtils::GetValue(env, panelFlagObj, panelFlag);//JsSelectionUtils::GetValue(env, panelFlagObj, panelFlag);
    }
