...
class selectionfwk_user_switch_0600(TestCase):
    ...
    def process(self):
        Step("验证多用户下，设置相同的划词应用，划词扩展拉起")
        Step("执行操作")
        driver = UiDriver.create(self.device1)

        Step("预置工作：安装划词扩展demo")
        driver.AppManager.install_app(r"testFile\SUB_SELECTIONFWK_BASE\system-signed.hap")
        driver.wait(2)

        Step("打开划词开关、切换划词扩展应用为com.selection.selectionapplication/SelectionExtensionAbility")
