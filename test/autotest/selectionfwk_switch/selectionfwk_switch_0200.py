...
class selectionfwk_switch_0200(TestCase):
    ...
    def process(self):
        ...
        Step("步骤2:关闭划词进程")
        driver.shell("param set sys.selection.switch off")
        driver.wait(20)

        Step("步骤3:查询结果")
        trigger_result = driver.shell("ps -ef | grep selection")
        keyword1 = "selection_service"
        keyword2 = "com.hxxxxi.xxos.vassistant:selection"
        is_exist = False
        if keyword1 in trigger_result or keyword2 in trigger_result:
            is_exist = True
        else:
            is_exist = False
